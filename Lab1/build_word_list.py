...
def build_word_list(filename: str) -> list[str]:
    ...
    >>> word_list
    >>> len(word_list)  # How many different words are in the file? 249
    """
...
-->Using a for loop, the program goes through the file line by line
-->All the words in the line are added to a list
-->Each word is stripped of punctuation and added to a set
-->After looping through all the lines, the set is the new list of words and is sorted
How does the function ensure that duplicate words aren't in the word list returned by the
function?
-->Uses a set data structure that doesn't contain duplicates
-->Every time a word is added to the set, it is ensured that it doesn't alr exist
How is the list sorted into ascending order?
-->Using the sort method which sorts in ascending order by default
"""
